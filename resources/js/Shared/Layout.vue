      ...
      <div class="md:h-screen md:flex md:flex-col">
        <div class="md:flex md:flex-shrink-0">
          ...
          <div class="bg-white border-b w-full p-4 md:py-0 md:px-12 text-sm md:text-md flex justify-between items-center">
            <div class="mt-1 mr-4">{{ $page.props.auth.user.first_name }}</div>
            <dropdown class="mt-1" placement="bottom-end">
              <div class="flex items-center cursor-pointer select-none group">
                <div class="flex text-gray-700 group-hover:text-indigo-600 focus:text-indigo-600 mr-1 whitespace-nowrap">
                  <span>{{ $page.props.auth.user.first_name +" "+ $page.props.auth.user.last_name }}</span>
				   <img v-if="foto" class="block w-5 h-5 rounded-full mr-2 ml-2"  :src="'/img/'+foto" />
                </div>
...
export default {
  ...
  // $page прилетает с сервера в бладе была бы просто переменная, а тут летит сюда в пропс, для удобства $ ставим 
  //props: { 
  //  $page: Object,  
  //},
  data(){
    return { foto : this.$page.props.page.photo_path }
  },
  mounted(){
	console.log("---!!! "+this.$page.props.auth.user.first_name);
  }
}
</script>
